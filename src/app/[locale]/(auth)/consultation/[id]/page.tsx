...
export default async function ConsultationDetailPage({ params }: ConsultationDetailPageProps) {
  ...
  const tNav = await getTranslations("nav"); // For breadcrumbs
    ...
    <div className="container mx-auto px-4 py-8">
      <div className="mb-8">
        <nav className="flex items-center gap-2 text-sm text-muted-foreground mb-4">
          <Link href="/dashboard" className="hover:text-foreground">{tNav("home")}</Link>
          <span>/</span>
          <Link href="/consultations" className="hover:text-foreground">{t("myConsultations")}</Link>
          <span>/</span>
          <span className="text-foreground">{t("viewDetails")}</span>
        </nav>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <Card>
          ...
          <CardContent className="space-y-4">
            <div>
              <p className="text-sm text-muted-foreground">{t("treatmentDescription")}</p>
              <p className="mt-1">{consultation.treatment_description}</p>
            </div>

            <div>
              <p className="text-sm text-muted-foreground">{t("patientCount")}</p>
              <p className="mt-1">{consultation.patient_count}</p>
            </div>

            {consultation.travel_dates && (
              <div>
                <p className="text-sm text-muted-foreground">{t("travelDates")}</p>
                <p className="mt-1">
                  {(() => {
